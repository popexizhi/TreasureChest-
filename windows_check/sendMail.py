...
class Sender():
    ...
    def poster(self, data):
        ...
        try:
            #req = urllib2.Request(self._url, data_jsondumps, {'Content-Type': 'application/x-www-form-urlencoded'})
...
def backup_mail(fileph):
    """ file moves"""
    os.rename(fileph, "%s_%s" % (fileph, str(time.time())))
    print "backup ok"

if __name__ == "__main__":
    t = Sender("http://10.9.147.243/intra/public/sendEmail")
    filepath = "send_mail.txt"
    f=open(filepath)
    con = f.readlines()
    conx= ""
    f.close()
    ...
    res = t.poster(data)
    if 0 == res["response_code"]:
        print "send ok"
        backup_mail(filepath)
    else:
        print "send err"
